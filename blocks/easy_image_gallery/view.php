...
?>
    ...
    <?php foreach ($files as $key => $f) :
        ...
        <div class="box-wrap masonry-item b-col-<?php echo $imageColumn ?> <?php echo $fileTags[$f->getFileID()]['handle'] ?>">
            <?php if($options->lightbox) : ?><a href="<?php echo $fullUrl ?>" data-fancybox-group="easy-gallery-<?php echo $bID?>" data-image="<?php echo $fullUrl ?>" <?php if($options->lightboxTitle) : ?> title="<?php echo $f->getTitle() ?><?php if($options->lightboxDescription) : ?> <?php echo $f->getDescription() ?><?php endif ?>"<?php endif ?> ><?php endif ?>       
